...
class DifficultWord {
  ...
  DifficultWord({required this.word, required this.count, required this.lastSeen});
}
...
class DifficultWordsRepository {
  String _keyFor(String userId) => 'difficult_words:${userId.isEmpty ? 'guest' : userId}';
  ...
  Future<void> addWord({required String word, String userId = ''}) async {
    ...
    if (raw != null) {
      try { m = json.decode(raw) as Map<String, dynamic>; } catch (_) {}
    }
    final now = DateTime.now().toIso8601String();
    final entry = (m[word] as Map<String, dynamic>?) ?? {'count': 0, 'lastSeen': now};
    final next = (entry['count'] as num?)?.toInt() ?? 0;
    m[word] = {'count': next + 1, 'lastSeen': now};
    await prefs.setString(key, json.encode(m));
  }

  Future<List<DifficultWord>> topWords({String userId = '', int limit = 30}) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    try {
      ...
      final list = m.entries.map((e) {
        ...
        final ls = DateTime.tryParse((v['lastSeen'] as String?) ?? '') ?? DateTime.now();
  ...
  // --- Supabase sync -------------------------------------------------------
  static const _table = 'difficult_words'; // Columns: user_id uuid, word text, count int, last_seen timestamptz
  ...
  Future<void> flushToServer({required String userId}) async {
    ...
    for (final entry in local.entries) {
      ...
      final lLast = DateTime.tryParse((v['lastSeen'] as String?) ?? '') ?? DateTime.now();
      try {
        ...
        if (row != null) {
          ...
          final sLast = DateTime.tryParse((row['last_seen'] as String?) ?? '') ?? lLast;
  ...
  Future<List<DifficultWord>> topWordsDbFirst({required String userId, int limit = 30}) async {
        ...
        if (list.isNotEmpty) {
          return list.map((e) {
            ...
            return DifficultWord(
              ...
              lastSeen: DateTime.tryParse((m['last_seen'] as String?) ?? '') ?? DateTime.now(),
            );
          }).toList();
        }
      } catch (_) {}
